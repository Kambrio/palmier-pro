        ...
        AgentTool(
            ...
            inputSchema: objectSchema(
                properties: [
                    ...
                    "voice": ["type": "string", "description": "TTS only. Voice preset name. list_models shows voicesSample (first 3) + voiceCount; any voice supported by the model is accepted. Defaults to the model's defaultVoice. Ignored by music models. For the local omnivoice-local model, pass the mediaRef of an audio OR video clip whose voice to clone (e.g. a speaker in the footage); audio is extracted automatically and the local proxy is used when the source file is offline. Cloning overrides styleInstructions."],
                    "language": ["type": "string", "description": "TTS language for the local OmniVoice provider — a language name (English, Spanish, Russian) or ISO code (en, es, ru). Required for OmniVoice so the script is spoken in the right language. Ignored by cloud TTS models."],
                    ...
                    "styleInstructions": ["type": "string", "description": "Gemini TTS only. Optional delivery instructions (e.g. 'warm and slow', 'British accent'). For omnivoice-local voice DESIGN (when not cloning), only these tokens are accepted and anything else is rejected: female, male, child, elderly, middle-aged, british accent, american accent, australian accent, canadian accent, chinese accent, indian accent, japanese accent, korean accent, high pitch, low pitch, moderate. Combine with commas, e.g. 'female, british accent'."],
