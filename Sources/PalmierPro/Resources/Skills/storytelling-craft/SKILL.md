...
## Apply in Palmier
Storytelling is theory, but the arc maps onto the timeline: order clips so each scene reads arrival → exploration → leaving; place the cold open first; reserve the payoff. Use `inspect_timeline` / `get_transcript` to review whether the current cut has a tension thread, and `add_texts` for act/chapter cards that mark structure. If you produce a written story plan or structure notes the user wants to keep, save it with `save_document(filename, content, format: "md")` — it lands in the project's documents folder (`documents/` inside the .palmier project by default; configurable in Settings → Storage) and shows in the Library's **Documents** tab.
