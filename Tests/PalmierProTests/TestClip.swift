...
enum TestClip {
    ...
    static func makeRigidDotsClip(
        ...
        let sw = CGFloat(width), sh = CGFloat(height)
        // CI (bottom-left) dot positions at frame 0, and their centroid (rotation pivot).
        let ciDots = dots.map { CGPoint(x: $0.x * sw, y: (1 - $0.y) * sh) }
        ...
        for i in 0..<frames {
            ...
            var img = noise
            for ci in ciDots {
                // Rotate about centroid (in pixels), then translate in x.
                ...
                if i == seedFrame { seedPoints.append(CGPoint(x: rx / sw, y: 1 - ry / sh)) }
            }
            ctx.render(img.cropped(to: CGRect(x: 0, y: 0, width: width, height: height)), to: buffer)
            while !input.isReadyForMoreMediaData { try await Task.sleep(nanoseconds: 1_000_000) }
            adaptor.append(buffer, withPresentationTime: CMTime(value: CMTimeValue(i), timescale: 30))
        }
        input.markAsFinished()
